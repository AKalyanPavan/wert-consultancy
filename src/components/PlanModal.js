	...
	return (
		<ul className="list-disc ml-[60px] mt-[10px] flex flex-col gap-[10px] mb-[10px]">
			{subFeatures.map(function (subFeature, i) {
				return (
					<li key={uuidv4()}>{subFeature}</li> // Added unique `key` prop
				);
			})}
		</ul>
	);
	...
	return (
		<div className="p-[30px] rounded-[10px] relative">
			<div
				...
				className="font-bold w-fit sticky top-[30px] hover:bg-[#0077b6] bg-[#082d60] text-white rounded-[5px] cursor-pointer py-[5px] px-[10px] ml-auto"
			>
				X
			</div>
			...
			<div className="flex flex-col gap-[15px]">
				{plan.planFeatures.map(function (feature, i) {
					return (
						<div key={uuidv4()}> {/* Added unique `key` prop */}
